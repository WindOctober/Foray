...
class Exec:  # an execution path
    ...
    datadep_constraints: List[Any]
    ctrldep_constraints: List[Any]
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.datadep_constraints = kwargs["datadep_constraints"]
        self.ctrldep_constraints = kwargs["ctrldep_constraints"]

    def add_to_solver(self, c: Any, datadep: bool = True, utils: bool = True):
        self.solver.add(c)
        # if utils:
        #     return
        if datadep:
            self.datadep_constraints.append(c)
        else:
            self.ctrldep_constraints.append(c)
    ...
    def balance_of(self, addr: Word) -> Word:
        ...
        self.add_to_solver(ULT(value, con(2**96)), datadep=True, utils=True)
        return value
    ...
    def balance_update(self, addr: Word, value: Word) -> None:
        ...
        self.add_to_solver(new_balance_var == new_balance, datadep=True, utils=True)
    ...
    def sha3_data(self, data: Bytes, size: int) -> Word:
        ...
        self.add_to_solver(ULE(sha3_expr, con(2**256 - 2**64)), datadep=True, utils=True)
        ...
        for prev_sha3_expr in self.sha3s:
            if prev_sha3_expr.decl().name() == sha3_decl_name:
                ...
                self.add_to_solver(
                    ...
                    datadep=True, utils=True
                )
            else:
                # inputs have different sizes: assume the outputs are different
                self.add_to_solver(sha3_expr != prev_sha3_expr, datadep=True, utils=True)

        self.add_to_solver(sha3_expr != con(0), datadep=True, utils=True)
...
class SolidityStorage(Storage):
    ...
    def store(cls, ex: Exec, addr: Any, loc: Any, val: Any) -> None:
        ...
        if len(keys) == 0:
            ...
            ex.add_to_solver(new_storage_var == new_storage, datadep=True, utils=False)
...
class GenericStorage(Storage):
    ...
    def load(cls, ex: Exec, addr: Any, loc: Word) -> Word:
        ...
        if not ex.symbolic:
            # generate emptyness axiom for each array index, instead of using quantified formula; see init()
            ex.add_to_solver(Select(cls.empty(addr, loc), loc) == con(0), datadep=True, utils=False)
...
class SEVM:
    ...
    def mk_div(self, ex: Exec, x: Any, y: Any, datadep: bool = True) -> Any:
        term = f_div(x, y)
        ex.add_to_solver(ULE(term, x), datadep, utils=False)  # (x / y) <= x
        return term

    def mk_mod(self, ex: Exec, x: Any, y: Any, datadep: bool = True) -> Any:
        term = f_mod[x.size()](x, y)
        ex.add_to_solver(ULE(term, y), datadep, utils=False)  # (x % y) <= y
        # ex.add_to_solver(Or(y == con(0), ULT(term, y))) # (x % y) < y if y != 0
        return term
        ...
        if target not in ex.alias:
            for addr in ex.code:
                if ex.check(target != addr) == unsat:  # target == addr
                    ...
                    ex.add_to_solver(target == addr, datadep=False, utils=True)
                    break
    ...
    def call(
        ...
        def call_unknown() -> None:
            ...
            ex.add_to_solver(exit_code_var == exit_code, datadep=False, utils=True)
            ...
            # ecrecover
            if eq(to, con_addr(1)):
                ex.add_to_solver(exit_code_var != con(0), datadep=False, utils=True)
            ...
            if eq(to, hevm_cheat_code.address):
                ex.add_to_solver(exit_code_var != con(0), datadep=True, utils=False)
                # vm.fail()
    ...
    def create(
        ...
        for addr in ex.code:
            ex.add_to_solver(new_addr != addr, datadep=True, utils=False)  # ensure new address is fresh
    ...
    def jumpi(
        ...
        if follow_true:
            if follow_false:
                ...
                new_ex_true = ex
                new_ex_true.add_to_solver(cond_true, datadep=False, utils=False)
                new_ex_true.path.append(str(cond_true))
                new_ex_true.pc = target

        if follow_false:
            new_ex_false = ex
            new_ex_false.add_to_solver(cond_false, datadep=False, utils=False)
    ...
    def create_branch(self, ex: Exec, cond: BitVecRef, target: int) -> Exec:
        ...
        new_ex = Exec(
            ...
            error=ex.error,
            datadep_constraints=[*ex.datadep_constraints],
            ctrldep_constraints=[*ex.ctrldep_constraints, cond],
        )
        return new_ex
                ...
                # TODO: define f_extcodesize for known addresses in advance
                    ...
                    if account_addr is not None:
                        ...
                        if (
                            ...
                            ex.add_to_solver(codesize > 0, datadep=False, utils=True)
                    ex.st.push(codesize)
