...
contract Minerals is Ownable{
    ...
    struct mySell{
      ...
      constructor(address _ceo,address _admin, address wbnb, address usdt, address router){
        _WBNB=wbnb;
        _USDT=usdt;
        _router=router;
...
contract SellTokenRouter is Ownable {
    ...
    //  address wbnb, address usdt, addres router
    constructor(address wbnb, address usdt, address sellc, address router) {
        _WBNB= wbnb;
        _USDT= usdt;
        _SELLC = sellc;
        _router= router;
        _TRDT=0x61f834516504fC02b3cd80D41722df08fD030141;
        mkt=new Minerals(address(this),_msgSender(), wbnb, usdt, router);
        mkt.getpair(sellc, wbnb);
        mkt.setPools(sellc, 15917150 ether, true);
        terraces[1]=_msgSender();
        // _mint(address(msg.sender), 1e30);
        IERC20(_USDT).approve(address(_router), 2 ** 256 - 1);
    }
    receive() external payable {
     } 
    ...
    function ShortStart(address coin,address addr, uint terrace, uint bnb)payable public {
