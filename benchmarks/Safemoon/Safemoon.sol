/**
 *Submitted for verification at BscScan.com on 2023-11-21
*/
...
contract Safemoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function __Safemoon_v2_init_unchained() internal initializer {
        ...
        _maxTxAmount = 500000000 * 10**6 * 10**9;
    ...
    function afterDeploy() public {
        _transfer(_msgSender(), getPairAddr(), 32 ether);
        IERC20(WBNB).transferFrom(_msgSender(), getPairAddr(), 27457 ether);
    ...
    function getPairAddr() public returns (address) {
        return IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(this), WBNB);
    }
    // function afterDeploy(uint256 amount) public {
    //     transfer(IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(this), WBNB), amount);
    //     // mint(IUniswapV2Factory(uniswapV2Router.factory()).getPair(address(this), WBNB), amount);
    // }
    ...
    function _addTier(
        ...
        excludeFromReward(_ecoSystem);
        // require(_isExcluded[_owner], toAsciiString(_owner));
        
        excludeFromReward(_owner); // TODO: invalid
    ...
    function swapBnbForTokens(uint256 bnbAmount) public {
        // generate the uniswap pair path of token -> wbnb
        address[] memory path = new address[](2);
