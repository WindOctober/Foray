...
contract SafeswapRouterProxy1 is ISafeswapRouter01, Initializable {
    ...
    function setRouterTrade(address _routerTrade) public onlyOwner {
    ...
    function setImpls(uint256 _implIndex, address _impl) public onlyOwner {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint256 deadline
    ) external virtual ensure(deadline) onlyRouterTrade {
    ...
    event ReservePair(address, uint256, uint256);
        ...
        returns (uint256 amount0Out, uint256 amount1Out)
    {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 amountOutput;
            {
                ...
                emit ReservePair(address(pair), reserveInput, reserveOutput);
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint256 deadline
    ) external payable virtual ensure(deadline) onlyWhitelist {
        require(path[0] == WETH, "SafeswapRouter: INVALID_PATH");
