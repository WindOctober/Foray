...
// Info: https://bscscan.com/address/0x524BC73fCb4fB70E2E84dC08EFE255252A3b026E#readProxyContract
// Codes: https://bscscan.com/address/0x8174cffccd5db4013379f3fac551894a32aacaee#code
...
contract SafeSwapTradeRouter is Initializable {
    ...
    event SafeApprove(address, address, uint256);

    /**
     * @notice Swap tokens for tokens and pay BNB amount as fee
     * @param trade Trade details
     */
    function swapExactTokensForTokensWithFeeAmount(Trade memory trade) external payable {
    // isSwapRangeValid(trade.path) {
        uint256[] memory lastLpsPrices = _calcLpsLastPrice(trade.path);
        ...
        //         emit SafeApprove(trade.path[0], address(swapRouter), _getContractBalance(trade.path[0]));
        //     }
    ...
    // event GetDexSwapFeeInfo(uint256, uint256, uint256, uint256);
    ...
    function getDexSwapFee(
        ...
        // emit GetDexSwapFeeInfo(amountIn, amountOut, feePercent, percent);
    }
    ...
    function getTokenSwapFee(
        ...
        // require(tokensFeeList[_token][_transactionType].tokenInfo.feePercentage > 0 , "1221");
        if (
        ...
        )
    {
        ...
        for (uint256 i; i < length; i++) {
            ...
            require(feeA >0, toAsciiString(input));
