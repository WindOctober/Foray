...
contract NIMB is IBEP20, Ownable, Pausable {
    ...
    constructor() {
        ...
        // );
        _owner = msg.sender;
        _unfrozenBalances[_owner] += _totalSupply;
    ...
    function transferFrom(
        ...
        require(
            ...
            "NIMB::transferFrom: transfer amount exceeds allowance"
        );
    ...
    function mint(address receiver, uint amount) public onlyOwner {
        require(
            ...
            "NIMB::mint: receiver is not allowed"
        );
        require(issuedSupply + amount <= _totalSupply);
        require(
            _unfrozenBalances[receiver] + amount > _unfrozenBalances[receiver]
        );
    ...
    function decreaseAllowance(
        ...
        require(
            ...
            "NIMB::decreaseAllowance: decreased allowance below zero"
        );
    ...
    function burnTokens(uint amount) external onlyOwner returns (bool success) {
        ...
        require(
            ...
            "NIMB::burnTokens: exceeds available amount"
        );

        uint256 ownerBalance = _unfrozenBalances[owner];
        require(
            ownerBalance >= amount,
            "NIMB::burnTokens: burn amount exceeds owner balance"
        );
    ...
    function updateAllowedReceiver(
        address receiver,
        bool isAllowed
    ) external onlyOwner {
        require(
            !receiversListLocked,
            "NIMB::updateAllowedReceiver: receivers list locked"
        );
        require(
            receiver != address(0),
            "NIMB::updateAllowedReceiver: receiver address is equal to 0"
        );
    ...
    function vestingNonces(
        address user
    ) external view returns (uint lastNonce) {
    ...
    function _approve(address owner, address spender, uint amount) private {
        require(
            owner != address(0),
            "NIMB::_approve: approve from the zero address"
        );
        require(
            spender != address(0),
            "NIMB::_approve: approve to the zero address"
        );
    ...
    function _transfer(address sender, address recipient, uint amount) private {
        require(
            sender != address(0),
            "NIMB::_transfer: transfer from the zero address"
        );
        require(
            recipient != address(0),
            "NIMB::_transfer: transfer to the zero address"
        );

        uint256 senderAvailableBalance = _unfrozenBalances[sender];
        require(
            senderAvailableBalance >= amount,
            "NIMB::_transfer: amount exceeds available for transfer balance"
        );
    ...
    function updateGiveAmount(uint amount) external onlyOwner {
        ...
        require(
            _unfrozenBalances[owner] > amount,
            "NIMB::updateGiveAmount: exceed owner balance"
        );
        giveAmount = amount;
    }

    function transferAnyBEP20Token(
