...
interface relationship {
    ...
    function otherCallSetRelationship(
        address _son,
        address _father,
        uint256 _amount
    ) external;
...
contract Zoom is ERC20Zoom, OwnableZoom {
    ...
    constructor() {
        ...
        _symbol = "$ZOOM";
        _decimals = 18;
        startTradeTime = block.timestamp - 1;
    ...
    function setPair(address[] memory _addr, bool _isUSDT) public onlyOwner {
        for (uint256 i = 0; i < _addr.length; i++) isPair[_addr[i]] = _isUSDT;
    }

    function setBlackListBat(
        address[] memory _addr,
        bool _YorN
    ) external onlyOwner {
        for (uint256 i = 0; i < _addr.length; i++) {
            blackList[_addr[i]] = _YorN;
        }
    }
