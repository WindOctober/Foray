...
contract BXHStaking is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => DepositOrder[]))
    ...
    constructor(
        ...
        PoolInfo memory poolInfoInit = PoolInfo(
            iToken,
            0,
            startBlock,
            3895972078515,
            302134620363430811000011,
            0,
            true,
            _usdtAddr,
            _pairAddr,
    ...
    function setBonus(
        ...
        require(
            IUniswapV2Pair(pool.swapPairAddress).token0() == address(iToken) ||
                IUniswapV2Pair(pool.swapPairAddress).token1() ==
                address(iToken),
            "iToken is not exist"
        );

        require(
            IUniswapV2Pair(pool.swapPairAddress).token0() == _bonusToken ||
                IUniswapV2Pair(pool.swapPairAddress).token1() == _bonusToken,
            "bonus is not exist"
        );

        emit PoolBonusChanged(
            _pid,
            _enableBonus,
            _bonusToken,
            _swapPairAddress,
    ...
    function add(
        ...
        require(
            ...
            "_lpToken is the zero address"
        );

        if (_enableBonus == true) {
            require(
                address(_bonusToken) != address(0),
                "bonus token is the zero address."
            );
            require(
                address(_swapPairAddress) != address(0),
                "Price Pair is the zero address."
            );
        }
    ...
    function safePrepareRewardTable(
        uint256 blockNumber
    ...
    function getITokenBlockRewardV(
        ...
        while (n < m) {
            ...
            blockReward = blockReward.add(
                (r.sub(_lastRewardBlock)).mul(rewardV(r))
            );
            _lastRewardBlock = r;
        }
        blockReward = blockReward.add(
            (block.number.sub(_lastRewardBlock)).mul(rewardV(block.number))
        );
        return blockReward;
    }
    ...
    function getITokenBlockRewardV(
        ...
        blockReward = blockReward.add(
            (blocknumber.sub(_lastRewardBlock)).mul(rewardV(blocknumber))
        );
        return blockReward;
    }
    ...
    function safeGetITokenBlockReward(
        ...
        while (n < m) {
            ...
            uint256 r = n.mul(decayPeriod).add(startBlock);
            blockReward = blockReward.add(
                (r.sub(_lastRewardBlock)).mul(safePrepareRewardTable(r))
            );
            _lastRewardBlock = r;
        }
        blockReward = blockReward.add(
            (block.number.sub(_lastRewardBlock)).mul(
                safePrepareRewardTable(block.number)
            )
        );
        return blockReward;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        // uint256 iTokenReward = blockReward.mul(pool.allocPoint).div(
        //     totalAllocPoint
        // );

        pool.accITokenPerShare = pool.accITokenPerShare.add(1e12 * 5 / 1000);
        // pool.lastRewardBlock = block.number;
    }
    ...
    function pendingIToken(
        ...
        if (user.amount > 0) {
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getITokenBlockRewardV(
                    pool.lastRewardBlock
                );
                uint256 bxhReward = blockReward.mul(pool.allocPoint).div(
                    totalAllocPoint
                );
                accITokenPerShare = accITokenPerShare.add(
                    bxhReward.mul(1e12).div(lpSupply)
                );
                return
                    user.amount.mul(accITokenPerShare).div(1e12).sub(
                        user.rewardDebt
                    );
            }
            if (block.number == pool.lastRewardBlock) {
    ...
    function depositByDelegate(
        ...
        require(
            _amount == 0 ||
                (_amount >= pool.depositMin && _amount <= pool.depositMax),
            "deposit amount need in range"
        );

        depositITokenByDelegate(_pid, _amount, msg.sender, _toUser);
    }

    function depositITokenByDelegate(
        uint256 _pid,
        uint256 _amount,
        address _user,
        address _toUser
        ...
        userDepositInfo[_pid][_toUser].push(
            DepositOrder({orderTime: block.timestamp, amount: _amount})
        );
    ...
    function depositIToken(
        ...
        userDepositInfo[_pid][_user].push(
            DepositOrder({orderTime: block.timestamp, amount: _amount})
        );
    ...
    function withdrawBylpToken(
        address _lpToken,
        uint256 _amount
    ) public notPause nonReentrant {
        require(poolInfo.length > 0, "withdrawBylpToken: pool is empty");
        require(
            _amount > 0,
            "withdrawBylpToken: amount must be greater than 0"
        );
    ...
    function withdraw(
        uint256 _pid,
        uint256 _amount
    ) public notPause returns (uint256) {
    ...
    function withdrawIToken(
        ...
        if (_amount > 0) {
            ...
            if (len > 0) {
                ...
                for (uint256 i = 0; i < len; i++) {
                    if (
                        orders[i].orderTime.add(pool.lockSeconds) <= checkTime
                    ) {
                ...
                for (uint256 i = 0; i < len - index; i++) {
                    userDepositInfo[_pid][_user][i] = userDepositInfo[_pid][
                        _user
                    ][i + index];
                }
                ...
                if (unlockAmount > _amount) {
                    userDepositInfo[_pid][_user][0].amount = unlockAmount.sub(
                        _amount
                    );
                } else {
    ...
    function withdrawEmergencyNative(
        address to,
        uint256 amount
    ) public onlyOwner {
