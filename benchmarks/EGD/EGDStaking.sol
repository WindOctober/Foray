...
contract EGDStaking is OwnableUpgradeable {
    ...
    constructor(address egdAddr, address usdtAddr, address routerAddr, address pairAddr) {
        rate = [200, 180, 160, 140];
        startTime = block.timestamp;
        referRate = [6, 3, 1, 1, 1, 1, 1, 1, 2, 3];
        rateList = [547, 493, 438, 383];
        ...
        EGD = IERC20(egdAddr);
        U = IERC20(usdtAddr);
        router = IUniswapV2Router(routerAddr);
    ...
    function _processRefer(address addr, uint amount) internal {
        ...
        for (uint i = 0; i < 10; i++) {
            ...
            if (
                (userInfo[temp].refer >= i + 1 &&
                    userInfo[temp].totalAmount >= 100 ether) ||
                userInfo[temp].isRefer
            ) {
    ...
    function getUserReferReward(address addr) public view returns (uint, uint) {
        return (
            userInfo[addr].referReward,
            userDailyReferReward[addr][getCurrentDay()]
        );
    }
    ...
    function stake(uint amount) external {
        ...
        userSlot[msg.sender][stakeId].rates =
            (amount * tempRate) /
            100000 /
            86400;
